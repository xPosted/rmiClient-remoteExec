...
public class XorSocket extends Socket {
    ...
    public synchronized InputStream getInputStream() throws IOException {
        if (in == null) {
            in = new XorInputStream(new BufferedInputStream(super.getInputStream()), pattern);
        }
        return in;
    }
    ...
    public synchronized OutputStream getOutputStream() throws IOException {
        if (out == null) {
            out = new XorOutputStream(new BufferedOutputStream(super.getOutputStream()), pattern);
        }
        return out;
    }
}